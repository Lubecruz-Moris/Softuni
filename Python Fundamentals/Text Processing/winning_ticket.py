...
def ticket_validator(ticket):
    ...
    if len(ticket) != 20:
        ...
        number_of_special_chars = data_source[0]
        special_sign = data_source[1]

        if number_of_special_chars < 6 or special_sign not in "@#$^":
            ticket_condition = f'ticket "{ticket}" - no match'
        else:
            ticket_condition = f'ticket "{ticket}" - {number_of_special_chars}{special_sign}'
